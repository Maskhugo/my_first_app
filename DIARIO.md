...
Task 4 - < 🔴 / 🟡 / 🟢 (preencher) > Transformei a ListaFilmesPage em StatefulWidget e a tela finalmente ficou interativa: digito o nome no TextField, toco no + e o filme aparece na lista na hora. O ponto principal foi entender por que o StatelessWidget não servia mais — a lista agora muda enquanto o app roda, então ela precisa morar no State (e não dentro do build, senão seria recriada a cada redesenho). O setState é o aviso de "mudei o dado, redesenha a tela": sem ele o filme entraria na lista mas a tela continuaria igual. O TextEditingController é o fio que liga o campo ao código, é por ele que eu leio o texto digitado e limpo o campo depois. E o if (texto.isEmpty) return; me mostrou o valor de validar antes de agir, em vez de deixar entrar um item vazio na lista. Na estrutura, a tela virou uma Column com o campo em cima e o ListView embaixo dentro de um Expanded, porque o ListView precisa de um espaço definido para ocupar o que sobra.